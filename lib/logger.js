...
module.exports = function Logger(config, log4js) {
    ...
    Object.defineProperty(Error.prototype, 'toObject', {
        value: function () {
            ...
            Object.getOwnPropertyNames(logger).forEach(function (key) {
                alt[key] = this[key];
            }, this);
    ...
    function formatJSONCSVLine(req, exceptionConstant, errorData) {
        ...
        if (config.log.errorData && errorData) {
            ...
            let error = JSON.stringify(errorObject);
            error = error.replace(";", " ");
            error = error.replace(/\n/g, " ");
            text += error;
        }
    ...
    logger.rcLog = function (req, logType, exceptionConstant, errorData) {
        let entry = formatJSONCSVLine(req, exceptionConstant, errorData);
        switch (logType) {
