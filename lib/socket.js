    ...
    if(tooBusy()){
        ...
        if(config.isTestEnvironment()) {
            ...
            if(handshakeData._query){
                if (handshakeData._query.tinychat == "true" &&
                    handshakeData._query.chattiny == "true") {

                    handshakeData.tinyName = handshakeData._query.tinyName;
                }
            }
        }

        //if this connection supplies identity information check it here and refuse it if invalid
        if(handshakeData._query.sessionKeyID !== undefined && handshakeData._query.encrypted !== undefined) {
            var sessionObj = {
                sessionKeyID: parseInt(handshakeData._query.sessionKeyID),
                encrypted: handshakeData._query.encrypted
            };
...
io.sockets.on('connection', function (socket) {
    ...
    if(socket.handshake.session) {
        ...
        socket.on('joinRemoteCon', function(user, sever, conversationID) {
            ...
            if(!config.validations.server.regExp.test(server) ||
                !config.validations.uuidV4.regExp.test(conversationID) ||
                !config.validations.user.regExp.test(user)){
                socket.close();
            } else {
                var mySocket;
