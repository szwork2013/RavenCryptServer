...
io.sockets.on('connection', function (socket) {
    ...
    if(socket.handshake.session) {
        ...
        socket.on('joinRemoteCon', function(user, sever, user, conversationID) {
            ...
            var count = socket.count('/socketRemoteCon/' + user + "/" + conversationID);
            ...
            if(count = 0){
                ...
                remoteSocket.on('connect', function(){
                    ...
                    remoteSocket.on('disconnect', function(){
                        var count = remoteSocket.count('/socketRemoteCon/' + user + "/" + conversationID);
                        if(count != 0) {
                            reconnect()
                        }
                    });
            ...
            socket.join('/socketRemoteCon/' + user + "/" + conversationID);
        });
    }
