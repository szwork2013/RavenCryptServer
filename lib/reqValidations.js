    ...
    this.run = function(req, cb){
        var internalCB = function(err){
            if(err){
                ...
                var check = scope.checks.splice(0, 1);
    ...
    check.add(function(req, cb) {
        if(req.body.encryptedPrivateKey) {
            ...
            var localCB = function(err){
                if(err){
                    cb(err);
                } else if (innerChecks.length > 0) {
                    var check = innerChecks.splice(0, 1);
                    check(req, localCB);
                } else {
                    cb();
                }
...
module.exports.blogList = function () {
    var check = new RCCheck();
    check.add(checkParamsUser(req, "user"));
    check.add(checkParamsUUIDV4(req, "id"));
    check.add(checkQueryDate(req, "since"));
    check.add(checkQueryDate(req, "before"));
    return check;
};
...
module.exports.blogRelationGetDelete = function () {
    var check = new RCCheck();
    check.add(checkParamsUser(req, "user"));
    check.add(checkParamsUser(req, "name"));
    check.add(checkParamsServer(req, "server"));
    return check;
};
...
module.exports.userMsgReceive = function () {
    ...
    check.add(checkParamsUser(req, "user"));
    check.add(checkParamsUUIDV4(req, "id"));
    check.add(reqBodyisType(req.body, "text", "string"));
    check.add(reqBodyisType(req.body, "keyID", "string"));
    check.add(function(req, cb) {
        helper.hasExactProperties(
            req.body, [
                "text",
...
module.exports.userKeyLogin = function () {
    ...
    check.add(checkParamsUser("user"));
    check.add(checkParamsKeyID("id"));
    check.add(function (cb, req) {
        if(req.query.onlyWatch){
            reqBodyisType(req.body, "onlyWatch", "boolean")(req, cb);
        }
    });
    return check;
};