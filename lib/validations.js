 ...
 */
module.exports = function validations(config, constants, openpgp) {
    ...
    validations.RCOptions = function (pgpMessage) {
        var msg = validations.readSignedClearTextMessage(pgpMessage);
        try {
            var msgContent = JSON.parse(msg.text);
        } catch (err) {
            throw constants.constants.optionsNoJson;
        }
        helper.hasExactPropertiesException(msgContent, ["encryptionID", "storeKeysOnServer"]);
        if (typeof msgContent.encryptionID !== "string") {
            throw "encryptionID is no string"; //
        }
        if (typeof msgContent.storeKeysOnServer !== "boolean") {
            throw "storeKeysOnServer is no boolean"; //
        }
        return msgContent;
    };
