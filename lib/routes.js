...
app.post('/user/:user/blog/:id', reqHandler({json: true, auth: true, checkSession: true, next: reqValidator(reqValidations.blogPost)}), routes.blog.post);
app.delete('/user/:user/blog/:id', reqHandler({auth: true, checkSession: true, next: reqValidator(reqValidations.blogGetDelete)}), routes.blog.delete);
...
app.post('/user/:user/relation/:server/:name', reqHandler({json: true, auth: true, checkSession: true, next: reqValidator(reqValidations.blogRelationUpsert)}), routes.blogRelation.upsert);
app.delete('/user/:user/relation/:server/:name', reqHandler({auth: true, checkSession: true, next: reqValidator(reqValidations.blogRelationGetDelete)}), routes.blogRelation.delete);
...
app.post('/user/:user/blog/:blogID/response/:server/:name/:id', reqHandler({json: true, next: reqValidator(reqValidations.blogResponsePost)}), routes.blogResponse.post);
...
app.post('/user/:user/blog/:blogID/response/:server/:name/:id/review', reqHandler({auth: true, checkSession: true, next: reqValidator(reqValidations.blogResponseReview)}), routes.blogResponse.review);
app.get('/user/:user/blog/:blogID/response', reqHandler({optionalAuth: true, next: reqValidator(reqValidations.blogResponseList)}), routes.blogResponse.list);
...
app.post('/user/:user/storage/:id', reqHandler({json: true, auth: true, checkSession: true, next: reqValidator(reqValidations.userStorageUpsert)}), routes.userStorage.upsert);
app.delete('/user/:user/storage/:id', reqHandler({auth: true, checkSession: true, next: reqValidator(reqValidations.userStorageGetDelete)}), routes.userStorage.delete);
app.get('/user/:user/storage/:id', reqHandler({auth: true, allowWatch: true, next: reqValidator(reqValidations.userStorageGetDelete)}), routes.userStorage.get);
app.get('/user/:user/storage', reqHandler({auth: true, allowWatch: true, next: reqValidator(reqValidations.userStorageList)}), routes.userStorage.list);
...
app.post('/user/:user/con/:id/manifest', reqHandler({json: true, auth: true, checkSession: true, next: reqValidator(reqValidations.conversationManifestUpsert)}), routes.conversationManifest.upsert);
...
app.delete('/user/:user/con/:id/manifest', reqHandler({auth: true, checkSession: true, next: reqValidator(reqValidations.conversationManifestGetDelete)}), routes.conversationManifest.delete);
...
app.post('/user/:user/con/:conID/msg/:id', reqHandler({json: true, next: reqValidator(reqValidations.conMsgReceive)}), routes.conversationMessage.receive);
...
routes.user.encryptionGet = route("user/encryptionGet.js");
routes.user.encryptionUpdate = route("user/encryptionUpdate.js");
...
app.post('/user/:user/profile', reqHandler({json: true, auth: true, next: reqValidator(reqValidations.userProfileUpdate)}), routes.user.profileUpdate);
app.get('/user/:user/encryption', reqHandler({next: reqValidator(reqValidations.userEncryptionGet)}), routes.user.profileGet);
app.post('/user/:user/encryption', reqHandler({json: true, auth: true, next: reqValidator(reqValidations.userEncryptionUpdate)}), routes.user.profileUpdate);
app.post('/user/:user', reqHandler({json: true, next: reqValidator(reqValidations.userRegister)}), routes.user.register);
app.post('/user/:user/confirmation', reqHandler({json: true, next: reqValidator(reqValidations.userRegisterConfirm)}), routes.user.registerConfirm);
...
app.post('/user/:user/key/:id', reqHandler({json: true, auth: true, checkSession: true, next: reqValidator(reqValidations.userKeyUpsert)}), routes.userKey.upsert);
app.post('/user/:user/key/:id/login', reqHandler({json: true, auth: true, checkSession: true, next: reqValidator(reqValidations.userKeyMakeLogin)}), routes.userKey.makeLogin);
...
app.post('/user/:user/msg/:id', reqHandler({json: true, next: reqValidator(reqValidations.userMsgReceive)}), routes.userMessage.receive);
app.delete('/user/:user/msg/:id', reqHandler({auth: true, checkSession: true, next: reqValidator(reqValidations.userMsgGetDelete)}), routes.userMessage.delete);
app.get('/user/:user/msg/get/:id', reqHandler({auth: true, allowWatch: true, next: reqValidator(reqValidations.userMsgGetDelete)}), routes.userMessage.get);
app.get('/user/:user/msg', reqHandler({auth: true, allowWatch: true, next: reqValidator(reqValidations.userMsgList)}), routes.userMessage.list);
...
app.post('/user/:user/upload/:id', reqHandler({json: true, auth: true, checkSession: true, passThrough: true, next: reqValidator(reqValidations.uploadGet)}), routes.upload.upload);
app.delete('/user/:user/upload/:id', reqHandler({auth: true, checkSession: true, next: reqValidator(reqValidations.uploadDelete)}), routes.upload.delete);
