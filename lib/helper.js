...
module.exports = function (openpgp) {
     ...
     * @param obj, propertyOne, propertyTwo...
     * @returns {boolean}
     */
     ...
     * @param obj, propertyOne, propertyTwo... N
     * @returns {boolean}
     */
    this.hasExactPropertiesException = function (obj, props) {
        for (var prop in obj) {
    ...
    this.hasExactProperties = function (obj, props, cb) {
        for (var prop in obj) {
            var index = props.indexOf(prop);
            if (index == -1) {
     ...
     */
    this.pgpEncrypt = function (text, publicKeyArmored) {
     ...
     */
    this.pgpDecrypt = function (text, privateKeyArmored) {
    ...
    this.toArrayBuffer = function (buffer) {
    ...
    this.toArrayBufferBin = function (buffer) {
        return new Uint8Array(buffer).buffer;
    };

    this.toBufferBin = function (ab) {
